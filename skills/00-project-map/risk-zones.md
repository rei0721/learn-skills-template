...
| internal/service/rbac/  | 权限风险     | 🟠 高    | RBAC 业务逻辑，错误可能导致越权访问    |
| internal/service/auth/  | 安全风险     | 🟠 高    | 登录注册、发 Token，错误可能导致越权   |
| internal/handler/rbac*  | 安全风险     | 🟠 高    | RBAC 管理接口，错误可能导致权限被篡改  |
| internal/handler/auth*  | 安全风险     | 🟠 高    | 登录注册入口，错误可能导致撞库/绕过    |
...
#### 9. internal/service/rbac & internal/handler/rbac - RBAC 管理接口

**风险描述**:
- RBAC 管理接口暴露后，任何绕过认证/鉴权的缺陷都会直接导致权限被篡改
- RBAC 变更如果未持久化或缓存未失效，可能出现“看起来成功但不生效”的问题

**操作约束**:
- ✅ 允许: 调整接口参数校验与错误码映射
- ✅ 允许: 增加审计日志（角色/策略变更）
- ⚠️ 谨慎: 调整路由鉴权策略（必须确保仅管理员可调用）
- ❌ 禁止: 对外开放 /enforce 作为通用鉴权绕过点

**测试要求**:
- 必须测试 RBAC handler 的参数校验与错误处理
- 必须测试 RBAC service 的核心校验逻辑
